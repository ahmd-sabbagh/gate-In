...
function EditProjects() {
  ...
  const getData = () => {
    axios
      .get(`${basedUrl}/business-pioneer/projects/${prams.Id}`, {
        headers: {
      ...
      .then(({ data }) => {
        ...
        setTitle_ar(data.data.translates.title_ar);
        setProjectTips(data.data.types_project_id);
        setIndustrial(data.data.industrial);
        // طبيعة المشروع
        setAgricultural(data.data.agricultural);
        setCommercial(data.data.commercial);
        setDescription_ar(data.data.translates.description_ar);
        // ماهو الحل لمشكلة المستخدم
        setTechnical_means(data.data.technical_means);
        setFinancial_resources(data.data.financial_resources);
        setMaterial_means(data.data.material_means);
        setHuman_resources(data.data.human_resources);
        setPublications(data.data.publications);
        setMedia(data.data.media);
        setGovernment_approvals(data.data.government_approvals);
        // بماذا يتميز المشروع ؟
        setprice(data.data.price);
        setQuality(data.data.quality);
        setDelivery_service(data.data.delivery_service);
        setSaving_speed(data.data.saving_speed);
        setMobile_app(data.data.mobile_app);
        setEncouragement_incentives(data.data.encouragement_incentives);
        // الفئة المستهدفة
        setTarget(data.data.target_group);
        // هل تحتاج الى مستثمر
        setInvestor(data.data.need_investor);
        // الرقعة الجغرافية
        setCountries(data.data.country_id);
        setGoverment(data.data.province_id);
        setCity(data.data.city_id);
        // اعضاء الفريق
        const Array = [];
        data.data.team.map((item) => Array.push(item));
        setFormValues(Array);
      })
      .catch((error) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        `${basedUrl}/business-pioneer/projects/update/${prams.Id}`,
        formData,
        {
          headers: {
            Accept: "application/json",
